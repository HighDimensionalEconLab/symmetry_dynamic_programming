...
plt.fill_between(quant_result_identity["t"], quant_result_identity["quantile_0.1"], quant_result_identity["quantile_0.9"], color='cornflowerblue', alpha=0.2, label = r"$10$th and $90$th percentiles")
plt.fill_between(quant_result_identity["t"], quant_result_identity["quantile_0.25"], quant_result_identity["quantile_0.75"], color='cornflowerblue', alpha=0.6,  label = r"$25$th and $75$th percentiles")
plt.title(r"Policy errors ($\epsilon_{\mathrm{rel}}$) with $\phi($Identity$)$")
plt.xlabel(r"Time($t$)")
...
plt.title(r"Policy errors ($\epsilon_{\mathrm{rel}}$) with $\phi($Moments$)$")
plt.xlabel(r"Time($t$)")
plt.legend(prop={"size": fontsize}, loc='lower left')
plt.tight_layout()
...
plt.title(r"Policy errors ($\epsilon_{\mathrm{rel}}$) with $\phi($ReLU$)$")
plt.xlabel(r"Time($t$)")
plt.legend(prop={"size": fontsize}, loc='lower left')
plt.tight_layout()
