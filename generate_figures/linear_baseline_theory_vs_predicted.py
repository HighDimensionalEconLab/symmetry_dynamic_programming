...
dfs={'Identity':"baseline_identity_one_run",'Moments':"baseline_deep_moments_one_run",'ReLU':"baseline_deep_sets_one_run"}
for name, tag in dfs.items():
    ...
    plt.plot(df["t"], df["u_hat"], label=fr"$u(X_t)$, $\phi(${name}$)$")
